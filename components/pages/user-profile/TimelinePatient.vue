...
function separateAppointments(appointments: any) {
    for (const appointment of appointments) {
        for (const patient of appointment.appointments) {
            if (isPastAppointment(appointments.day, appointments.timeRange)) {
...
onMounted(async () => {
    ...
    await meetStore.fetchPatientAppointments();
    ...
    <div class="timeline-container text-center" >
        ...
        <v-timeline align="start" side="end" v-if="appointments">
            <v-timeline-item dot-color="warning" size="small" v-for="(appointment, index) in future">
                <div class="d-flex" v-for="(i, index) in appointment.appointments">
                    <div>
                        <div>
                            <strong class="float-left">{{ i.day}}</strong>
                            <span> | {{ i.timeRange }}.00</span>
                        </div>
                        <div class="text-h6 font-weight-regular float-left truncate text-start" style="width: 200px;">
                            {{ i.doctorName }} {{ i.doctorSurname }}
                        </div>
                    </div>
                </div>
            </v-timeline-item>
